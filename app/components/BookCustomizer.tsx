...
const extras = {
  fr: [{ id: 'copy', name: 'Copie famille', detail: 'Un deuxième exemplaire identique', price: 49 }, { id: 'film', name: 'Mini-film « Notre maison »', detail: '1 à 3 minutes, photos et vidéos', price: 79 }, { id: 'memory', name: 'Memory Link + carte QR', detail: 'Galerie privée, récit, voix et film', price: 39 }, { id: 'frame', name: 'Cadre numérique préchargé', detail: 'Vos photos déjà installées', price: 129 }],
  en: [{ id: 'copy', name: 'Family copy', detail: 'A second identical book', price: 49 }, { id: 'film', name: '“Our Home” mini-film', detail: '1–3 minutes from photos and videos', price: 79 }, { id: 'memory', name: 'Memory Link + QR card', detail: 'Private gallery, story, voices and film', price: 39 }, { id: 'frame', name: 'Preloaded digital frame', detail: 'Your photos already installed', price: 129 }],
};
...
export function BookCustomizer({ locale = 'fr' }: { locale?: BookLocale }) {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { try { const saved = localStorage.getItem(`${STUDIO_STORAGE_KEY}-${locale}`); if (!saved) return; const draft = JSON.parse(saved); setTitle(draft.title || t.title); setAddress(draft.address || ''); setYears(draft.years || ''); setColor(draft.color || 'forest'); setFormat(draft.format || t.format[0]); setAnswers(draft.answers || {}); setSelectedExtras(draft.selectedExtras || ['memory']); } catch { /* start fresh */ } }, 0); return () => window.clearTimeout(timer); }, [locale, t.format, t.title]);
