...
export function SiteFooter({ locale = 'fr' }: { locale?: 'fr' | 'en' }) {
  ...
  return (
    <footer className="site-footer">
      ...
      <div><h4>{en ? 'Professionals' : 'Professionnels'}</h4><Link href={en ? '/en/professionals' : '/professionnels'}>{en ? 'Realtor offer' : 'Offre courtiers'}</Link>{!en ? <Link href="/admin">Administration</Link> : null}<a href="mailto:emilie@equipecauvier.com">{en ? 'Contact us' : 'Nous écrire'}</a></div>
