...
export function BookPreview({ locale = 'fr' }: { locale?: BookLocale }) {
  ...
  const approve = async () => {
    const versionHash = `${book.id}-v${book.version}-${book.updatedAt}`;
    if (book.projectId) await fetch('/api/approvals', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ projectId: book.projectId, versionHash }) });
